...
para.ICab_Scale             = pECC(20);
para.ICap_Scale             = pECC(21);
para.INCX_Scale             = pECC(22);
para.IClCa_Scale            = pECC(23);
para.IClb_Scale             = pECC(24);
para.Jrel_Scale             = pECC(25);
para.Jserca_Scale           = pECC(26);
para.Cleft_Buffer_Scale     = pECC(27);
para.Cytosol_Buffer_Scale   = pECC(28);
para.L_type_k1o             = pECC(29);
para.PLB_kmf                = pECC(30);
para.RyR_EC50               = pECC(31);
para.Jleak_Scale            = pECC(32);
para.CSQ_B_max              = pECC(33);
para.KiCa_scale             = pECC(34);
para.KoCa_scale             = pECC(35);
para.Kim_scale              = pECC(36);
para.Kom_scale              = pECC(37);


para.Na_clamp               = pECC(38);
para.Currents_record        = pECC(39);
para.Output_current         = pECC(40);


%Sex differences
gender_flag                 = pECC(41);
Female_GNa_ecc              = pECC(42);
Female_Gk1_ecc              = pECC(43);
Female_Gkur_ecc             = pECC(44);
Female_Gkach_ecc            = pECC(45);
Female_Ito_ecc              = pECC(46);
Female_CSQ_ecc              = pECC(47);
Female_NaK_ecc              = pECC(48);
Female_SK_ecc               = pECC(49);
DTE_flag                    = pECC(50);
Prot_interval_ECC           = pECC(51);

if gender_flag == 1

    Female_GNa_change           = Female_GNa_ecc;           
    Female_Gk1_change           = Female_Gk1_ecc;           
    Female_Gkur_change          = Female_Gkur_ecc;          
    Female_Gkach_change         = Female_Gkach_ecc;         
    Female_Ito_change           = Female_Ito_ecc;           
    Female_CSQ_change           = Female_CSQ_ecc;        
    Female_NaK_change           = Female_NaK_ecc;        
    Female_SK_change            = Female_SK_ecc;         

else

    Female_GNa_change           = 1;   
    Female_Gk1_change           = 1;   
    Female_Gkur_change          = 1;   
    Female_Gkach_change         = 1;   
    Female_Ito_change           = 1;   
    Female_CSQ_change           = 1;   
    Female_NaK_change           = 1;   
    Female_SK_change            = 1;   
...
% load opt23_factors_20190620_extended_K2_33_4_24_24 % LTCC Not included in
% 20% Baseline Reduction
% parameter_opt_AF = [1.00038034306119 1.00132194198757 0.901487093740190	1.12892583467646 1.33912767152642 0.910565596260301 0.352118269312542...
%      0.505286847108909 1.25940947767984 0.201612975655270 0.994862232915996 1.00438437285486 0.693393729482815 0.724121081291155];


parameter_opt_AF = [1 1 0.901487093740190 1.12892583467646 1.33912767152642 0.910565596260301 0.352118269312542...
     0.505286847108909 1.25940947767984 0.201612975655270 1 1 0.693393729482815 0.724121081291155];


% No LTCC in Opt
if AF == 1
    SA_par_AF = parameter_opt_AF; % Copy values from parameters_min_opt17 to SA_par
else
    SA_par_AF = ones(1,14);
end
...
if para.prot_index      == 1, simulation = 'pace_cc';
elseif para.prot_index  == 2, simulation = 'pace_cc_dte';
elseif para.prot_index  == 3, simulation = 'v_step';
elseif para.prot_index  == 4, simulation = 'pace_cc_dad';
elseif para.prot_index  == 100, simulation = 'pace_cc_erp';
end
...
GNa = (0.25 * para.kPKA_Ina_increase + 1) * (1 - 0.02 * AF) * SA_par(1) * 9; % [mS/uF] Courtemanche (adjusted for dV/dt max) MOD1
...
I_Na_junc = para.INa_Scale * Fjunc * GNa * Female_GNa_change * y(1)^3 * y(2) * y(3) * (y(39) - ena_junc);
I_Na_sl = para.INa_Scale * Fsl * GNa * Female_GNa_change * y(1)^3 * y(2) * y(3) * (y(39) - ena_sl);
...
% GNaL = para.INaL_Scale * ((14.0 / 11.0) ./ (1 + exp(-(para.NaV_CKp - 0.12) / 0.1))) * SA_par(2) * (1.2 * 0.003) * SA_par(1) * 9 ; % [mS/uF] (adjusted) MOD1 % AF changed by NH 11/21
...
IbarNaK = para.INaK_Scale * 1.0 * 1.26 * Female_NaK_change; % [uA/uF]
KmNaip = 11; % [mM]
...
GtoFast = 1.2 * 0.75 * para.Itof_Scale * 1.2 * (1 + (0.6 - 1) * para.kPKA_Itof) * SA_par(5) * ((1 - 0.7 * AF) * SA_par_AF(1)) * 0.165;
...
I_tof = Female_Ito_change * GtoFast * y(10) * y(11) * (y(39) - ek);
I_to = I_tof;
...
% GKr = 0.035 * sqrt(Ko / 5.4); % Added by NH
% gkr = para.IKr_Scale * (1 + para.dGkr_PKA) * SA_par(6) * 0.95 * 0.035 * sqrt(Ko / 5.4);
GKr = 0.035 * sqrt(Ko / 5.4) ; % Added by NH
...
I_kr = 1.1 * 0.9 * para.IKr_Scale * 0.35 * IKr_out * ((1 - 0.6 * AF) * SA_par_AF(2));
...
Gkur = 1.2 * para.IKur_Scale * para.kCKII_Ikur * (1.0 + 2 * para.kPKA_Ikur) * 1.8 * SA_par(8) * ((1 - 0.5 * AF) * SA_par_AF(3)) * 0.045 * (1 + 0.2 * RA);
...
I_kur = Gkur  * Female_Gkur_change * y(8) * y(9) * (y(39) - ek);
...
% if AF == 1
% 
%     IK1_mod = AF_modulator-1;
%     IK1_dec = 1-IK1_mod;
%     Gki =  Scale_IK1 * (0.75 * SA_par(9) * ((1 + 1 * AF)*IK1_dec) * (2.1 * 0.0525) * sqrt(Ko / 5.4)); %
% 
% elseif AF == 0
% 
%     Gki =  Scale_IK1 * (0.75 * SA_par(9) * (2.1 * 0.0525) * sqrt(Ko / 5.4)); %
% 
% end


% Gki =  Scale_IK1 * (0.75 * SA_par(9) * (1 + 0.5 * AF) * (2.1 * 0.0525) * sqrt(Ko / 5.4)); %
Gki =  Scale_IK1 * (0.75 * SA_par(9) * ((1 + 1 * AF) * SA_par_AF(4)) * (2.1 * 0.0525) * sqrt(Ko / 5.4)); %
...
I_ki_j = para.IK1_Scale * 1 / 1.31 * 0.65 * (0.15 + 0.85 ./ (1 + (y(32) / 10.0) .* (y(32) / 10.0))) * Fjunc * Gki * Female_Gk1_change * (y(39) - ek) / (1 + exp(0.07 * (y(39) - (ek + 6.94)))); % CRN IK1
I_ki_sl = para.IK1_Scale * 1 / 1.31 * 0.65 * (0.15 + 0.85 ./ (1 + (y(33) / 10.0) .* (y(33) / 10.0))) * Fsl * Gki * Female_Gk1_change * (y(39) - ek) / (1 + exp(0.07 * (y(39) - (ek + 6.94)))); % CRN IK1
...
gk2p = 0.9 * para.IK2p_Scale * 0.95 * SA_par(11) * K2P_cond * 0.0050 * (1 + 2 * AF); % AF changed by NH 11/21
...
I_kach_sl = I_kach_sl * Female_Gkach_change;
I_kach_j = I_kach_j * Female_Gkach_change;
...
gsk = 0.95 * para.ISK_Scale * 0.85 * SA_par(13) * SK_cond * par_sk(1) * ((1 + 1 * AF) * SA_par_AF(5)) * Female_SK_change; % Updated AF, NH 11/21
...
[jLTCC_tmp_ydot, jLTCC_out] =  LTCC_mode_1(jLTCC_tmp_state, y(36), y(39),  y(32), y(35), para.K_PKA_LTCC, Frdy, FoRT,  Cao, Nao, Ko, gender_flag, AF, para.L_type_k1o);
[jLTCC_tmp_ydot_m2, jLTCC_out_m2] =  LTCC_mode_2(jLTCC_tmp_state_m2, y(36), y(39),  y(32), y(35), para.K_PKA_LTCC, Frdy, FoRT,  Cao, Nao, Ko, gender_flag, AF, para.L_type_k1o);


[slLTCC_tmp_ydot, slLTCC_out] =  LTCC_mode_1(slLTCC_tmp_state, y(37), y(39),  y(33), y(35), para.K_PKA_LTCC, Frdy, FoRT,  Cao, Nao, Ko, gender_flag, AF, para.L_type_k1o);
[slLTCC_tmp_ydot_m2, slLTCC_out_m2] =  LTCC_mode_2(slLTCC_tmp_state_m2, y(37), y(39),  y(33), y(35), para.K_PKA_LTCC, Frdy, FoRT,  Cao, Nao, Ko, gender_flag, AF, para.L_type_k1o);
...
if gender_flag == 0
    ...
    I_Ca_junc = scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.9 * ( (1 - para.LTCC_junc_mode2) * jLTCC_out.I_Ca_junc_m1  + para.LTCC_junc_mode2 * jLTCC_out_m2.I_Ca_junc_m1) );
    I_CaNa_junc = scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.9 * ( (1 - para.LTCC_junc_mode2) * jLTCC_out.I_Na_junc_m1  + para.LTCC_junc_mode2 * jLTCC_out_m2.I_Na_junc_m1) );
    I_Ca_sl =  scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.1 * ( (1 - para.LTCC_sl_mode2) * slLTCC_out.I_Ca_junc_m1  + para.LTCC_sl_mode2 * slLTCC_out_m2.I_Ca_junc_m1) );
    I_CaNa_sl =  scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.1 * ( (1 - para.LTCC_sl_mode2) * slLTCC_out.I_Na_junc_m1  + para.LTCC_sl_mode2 * slLTCC_out_m2.I_Na_junc_m1) );
    I_CaK =  scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.9 * ( (1 - para.LTCC_junc_mode2) * jLTCC_out.I_K_junc_m1  + para.LTCC_junc_mode2 * jLTCC_out_m2.I_K_junc_m1) ...
        + 0.1 * ( (1 - para.LTCC_sl_mode2) * slLTCC_out.I_K_junc_m1  + para.LTCC_sl_mode2 * slLTCC_out_m2.I_K_junc_m1) );

elseif gender_flag == 1

    G_CaL_Scale = 0; % removed CaMKII-p instead
    I_Ca_junc = scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.9 * ( (1 - para.LTCC_junc_mode2) * jLTCC_out.I_Ca_junc_m1  + para.LTCC_junc_mode2 * jLTCC_out_m2.I_Ca_junc_m1) );
    I_CaNa_junc = scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.9 * ( (1 - para.LTCC_junc_mode2) * jLTCC_out.I_Na_junc_m1  + para.LTCC_junc_mode2 * jLTCC_out_m2.I_Na_junc_m1) );
    I_Ca_sl =  scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.1 * ( (1 - para.LTCC_sl_mode2) * slLTCC_out.I_Ca_junc_m1  + para.LTCC_sl_mode2 * slLTCC_out_m2.I_Ca_junc_m1) );
    I_CaNa_sl =  scale * (1 - G_CaL_Scale * AF) * para.G_LTCC * (0.1 * ( (1 - para.LTCC_sl_mode2) * slLTCC_out.I_Na_junc_m1  + para.LTCC_sl_mode2 * slLTCC_out_m2.I_Na_junc_m1) );
...
I_Catot = I_Ca + I_CaK + I_CaNa; %disp(I_Catot)

%% I_cabk: Ca Background Current

GCaB = para.ICab_Scale * 6.0643e-4 * SA_par_AF(9); % [uA/uF]

if MOD_ind == 3
    GCaB = para.ICab_Scale * 6.0643e-4 * SA_par_AF(9) ; % [uA/uF] MOD3 % Removed this AF Flag NH 11/21; Not sure why it is here
end
...
IbarNCX =  1 * SA_par(20) * ((1 + 0.4 * AF) * SA_par_AF(6)) * 3.15;

if (MOD_ind == 3)
    IbarNCX = 1.1 * 1.05 * 1.2 * SA_par(20) * ((1 + 0.4 * AF) * SA_par_AF(6)) * 3.15; % [uA/uF] MOD3; %NH Changed to 40% same as SK paper
...
if MOD_ind == 3
    Vmax_SRCaP = 1.1 * 1.5 * SA_par(21) * 5.3114e-3; % [mM/msec] MOD3; %

end

% Q10SRCaP = 2.6;          % [none]
if gender_flag == 0
    Kmf = para.PLB_kmf * PLB_kmf_Scale * (2.5) * 0.246e-3 * (1 - 0.1 * AF) * SA_par_AF(12); % [mM]
elseif gender_flag == 1
    Kmf = para.PLB_kmf * PLB_kmf_Scale * (2.5) * 0.246e-3 * SA_par_AF(12);
end


Kmr = 1.7;               % [mM]L cytosol
hillSRCaP = 1.787;       % [mM]
ks = 1.273543 * 1.1 * 25 * SA_par_AF(7); % [1/ms] %flux; % AF RyR Reduction

if gender_flag == 0
    koCa = 10 * para.KoCa_scale; % [mM^-2 1/ms]  % ISO will be modeled by PKA
elseif gender_flag == 1
%     koCa = 10 + 20 * AF; % [mM^-2 1/ms]  % ISO will be modeled by PKA
    koCa = para.KoCa_scale * (10 + 10 * AF); % [mM^-2 1/ms]  % ISO will be modeled by PKA

end

kom = 0.06 * SA_par_AF(14) * para.Kom_scale;              % [1/ms]

kiCa = para.KiCa_scale * 0.5 * SA_par_AF(8);              % [1/mM/ms]

kim = 0.005 * SA_par_AF(10) * para.Kim_scale;             % [1/ms]

if gender_flag == 0
    ec50SR = para.RyR_EC50 * 0.45 * SA_par_AF(13)  ;           % [mM]
elseif gender_flag == 1
    ec50SR = para.RyR_EC50 * 0.45 * (1 - 0.11 * AF) * SA_par_AF(13);           % [mM]
end
...
ydot(16) = (kiSRCa * y(36) * y(15) - kim * y(16)) - (kom * y(16) - KoSR_Ca_2 * RI); % C
J_SRCarel = para.Jrel_Scale * ks * y(15) * (y(31) - y(36)); % [mM/ms] RYR Release

%SERCA
J_serca = SA_par_AF(11) * para.Jserca_Scale * Vmax_SRCaP * ((y(38) / Kmf)^hillSRCaP - (y(31) / Kmr)^hillSRCaP) / (1 + (y(38) / Kmf)^hillSRCaP + (y(31) / Kmr)^hillSRCaP); % [mM/ms] Q10SRCaP^Qpow* was removed
%RyR
...
if gender_flag == 0 && AF == 1

    Bmax_Csqn = para.CSQ_B_max * 140e-3 * Vmyo / Vsr  * Female_CSQ_change * (1 - 0.45 * AF); %[mM] Csqn buffering

elseif gender_flag == 0 && AF == 0

    Bmax_Csqn = para.CSQ_B_max * 140e-3 * Vmyo / Vsr * Female_CSQ_change;

elseif gender_flag == 1 && AF == 1

    Bmax_Csqn = para.CSQ_B_max * 140e-3 * Vmyo / Vsr * Female_CSQ_change * (1 - 0.28 * AF); %[mM] Csqn buffering
...
switch lower(simulation)
    ...
    case 'pace_cc_dte',  % DTE
        %         model_flag = pacing_frequncies;
        first_beat = 7000;
        if t > first_beat && t < first_beat+5
            I_app = pacing_rate;
        elseif  t < first_beat
            I_app = 0.0;
        else
    ...
    case 'pace_cc_erp' % pace ERP protocol
        ...
        elseif t <= stim_time+5
            I_app = 2*DTE;
        elseif t > 5 && t <= Prot_interval_ECC
            I_app = 0.0;
        elseif t > Prot_interval_ECC && t <= Prot_interval_ECC+5
            I_app = DTE*2; %*2; % 2*DTE
        else
            I_app = 0.0;
        end
    ...
    case 'v_step' % use dependent block (UDB)
        %         rate = pacing_rate * 1e-3;
        %         V_test= para.Voltage_step;
        %         V_hold1 = -120; T_hold1 = 333;
        %         V_hold2 = V_test; T_hold2 = 40;
        ...
        %         rate = pacing_rate * 1e-3;
        V_hold1 = 0; T_hold1 = 200;
        V_hold1 = 0; T_hold1 = 200;
        V_hold2 = -80; T_hold2 = 100;
        if t > 10e3 &&  t < 10.5e3
            V_clamp = V_hold1;
        elseif t > 10e3 &&  t < 12.0e3
            V_clamp = V_hold2;
        end
        R_clamp = 0.02;
        I_app = (V_clamp-y(39))/R_clamp;
